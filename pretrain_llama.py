...
def model_provider(pre_process=True, post_process=True):
    ...
    args = get_args()
    if 'ENABLE_zero35' in os.environ:
        deepspeed_context = deepspeed.zero.Init(data_parallel_group=GroupMember.WORLD,
                             remote_device=None,
        ...
        deepspeed_context = deepspeed.zero.MiCS_Init(data_parallel_group=mpu.get_data_parallel_group(),
                             remote_device=None if args.remote_device == 'none' else args.remote_device,
                             config_dict_or_path=args.deepspeed_config,
                             enabled=args.zero_stage == 3,
                             mpu=mpu)

    with deepspeed_context:
        if args.deepspeed and not args.no_pipeline_parallel:
            ...
            # Predompute the attention mask and store it in args. This avoids having tox
...
if __name__ == "__main__":
    ...
    pretrain(random_train_valid_test_datasets_provider, model_provider, forward_step,
             args_defaults={'tokenizer_type': 'GPT2BPETokenizer'},#GPT2BPETokenizer SpmTokenizer
